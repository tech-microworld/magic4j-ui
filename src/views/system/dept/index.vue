...
<script>
import { listDeptTree, getDept, deleteDept, addDept, updateDept,treeSelect } from "@/api/system/dept";
...
export default {
  ...
  created() {
    this.getList();
    this.getDicts("dept_status").then(res => {
      this.statusOptions = [];
  ...
  methods: {
    /** 查询部门列表 */
    getList() {
      listDeptTree(this.queryParams).then(response => {
